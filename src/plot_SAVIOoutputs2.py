...
def get_nondimen_indices(sp, VPD, tmax):
    with open('../Si_'+sp+'_vpd'+str(int(VPD))+'_tmax'+str(tmax)+'_perPlantNL_outcomes.pickle', 'rb') as handle:
        Y = pickle.load(handle)
    with open('../Si_'+sp+'_vpd'+str(int(VPD))+'_tmax'+str(tmax)+'_perPlantNL_params.pickle', 'rb') as handle:
...
def barplot_twospecies(VPD, tmax):
    ...
    # this can less than the number of total variables analyzed
    # var_names = np.array(['A_canopy','Gs_leaf','c_leaf',
#                             'L_stem','A_stem','Ksat_stem',
#                             'a_stem','P50_stem','L_root',
#                             'A_root','Amax','rho'])

#     iplot = np.array([7,2,0,1,8,6,10,11])
    iplot=np.arange(len(var_names))
    ...
    ax.set_xticklabels([])
#     plt.ylim(0,1.5); 
    plt.xlim(-width, np.max(ind)+width*3.0)
